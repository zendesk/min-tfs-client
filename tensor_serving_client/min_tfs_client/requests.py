...
class AsyncTensorServingClient:
    def __init__(
        self,
        host: str,
        port: int,
        ...
        if credentials:
            self._channel = grpc.aio.secure_channel(target=self._host_address, credentials=credentials, options=options)
        else:
            self._channel = grpc.aio.insecure_channel(target=self._host_address, options=options)
    ...
    async def _make_inference_request(
        ...
        return await stub.Predict(request, timeout=timeout)
    ...
    async def predict_request(
        ...
        return await self._make_inference_request(**request_params)
    ...
    async def model_status_request(
        self,
        model_name: str,
        model_version: Optional[int] = None,
        ...
        if model_version:
            request.model_spec.version.value = model_version
        return await stub.GetModelStatus(request, timeout)
...
class TensorServingClient:
    def __init__(
        ...
        self._host_address = f"{host}:{port}"
        if credentials:
            self._channel = grpc.secure_channel(target=self._host_address, credentials=credentials, options=options)
        else:
            self._channel = grpc.insecure_channel(target=self._host_address, options=options)
